...
const exports = (module.exports = Yargs);
function Yargs(processArgs, cwd, parentRequire) {
  ...
  self.resetOptions = self.reset = function(aliases) {
    ...
    preservedGroups = Object.keys(groups).reduce((acc, groupName) => {
      const keys = groups[groupName].filter(key => key in globalLookup);
      if (keys.length > 0) {
        acc[groupName] = keys;
      }
      return acc;
    ...
    arrayOptions.forEach(k => {
      tmpOptions[k] = (options[k] || []).filter(k => globalLookup[k]);
    });

    objectOptions.forEach(k => {
      tmpOptions[k] = objFilter(options[k], (k, v) => globalLookup[k]);
    });
  ...
  self.demand = self.required = self.require = function(keys, max, msg) {
    ...
    if (typeof keys === 'number') {
      if (!options.demanded._)
        options.demanded._ = { count: 0, msg: null, max };
  ...
  self.parse = function(args, shortCircuit, _parseFn) {
    ...
    const parsed = parseArgs(args, shortCircuit);
    if (parseFn) {
  ...
  let helpOpt = null;
  let useHelpOptAsCommand = false; // a call to .help() will enable this
