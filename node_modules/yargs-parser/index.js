...
function parse(args, opts) {
  ...
  const __ =
    opts.__ ||
    function(str) {
      return util.format(...Array.prototype.slice.call(arguments));
    };
  let error = null;
  const flags = {
    aliases: {},
  ...
  Object.keys(opts.narg || {}).forEach(k => {
    flags.nargs[k] = opts.narg[k];
  });

  Object.keys(opts.coerce || {}).forEach(k => {
  ...
  if (Array.isArray(opts.config) || typeof opts.config === 'string') {
    ...
    Object.keys(opts.config || {}).forEach(k => {
  ...
  function eatNargs(i, key, args) {
    ...
    for (let ii = i + 1; ii < toEat + i + 1; ii++) {
      setArg(key, args[ii]);
    }
  ...
  function setArg(key, val) {
    ...
    if (
      ...
      keys.forEach(key => {
        argv.__defineSetter__(key, v => {
          val = path.normalize(v);
        });

        argv.__defineGetter__(
          key,
          () => (typeof val === 'string' ? path.normalize(val) : val),
        );
      });
    }
  }
  ...
  function setKey(obj, keys, value) {
    ...
    const isTypeArray = checkAllAliases(key, flags.arrays);
    const isValueArray = Array.isArray(value);
    const duplicate = configuration['duplicate-arguments-array'];
      ...
      Object.keys(obj || {}).forEach(key => {
        ...
        flags.aliases[key].forEach(x => {
          flags.aliases[x] = [key].concat(
            flags.aliases[key].filter(y => x !== y),
          );
        });
      });
    });
  ...
  function setDefaulted(key) {
    [].concat(flags.aliases[key] || [], key).forEach(k => {
  ...
  while (change) {
    ...
    for (let i = 0; i < aliasArrays.length; i++) {
      for (var ii = i + 1; ii < aliasArrays.length; ii++) {
        const intersect = aliasArrays[i].filter(
          v => aliasArrays[ii].indexOf(v) !== -1,
        );
